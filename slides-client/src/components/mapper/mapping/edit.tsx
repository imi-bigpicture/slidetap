import { Button, DialogActions, DialogContent, MenuItem, Select, SelectChangeEvent, Stack, TextField } from '@mui/material'
...
export function EditMapping (
    { attribute, setOpen, open }: EditMappingProps
): ReactElement {
...
function DisplayMapping (
    { mapping, mappableValue, onChange }: DisplayMappingProps
): ReactElement {
    let mappingFragment = (<></>)
...
function DisplayMappableValue (
    { mappableValue }: DisplayMappableValueProps
): ReactElement {
    if (mappableValue === undefined) {
        return <></>
    }
    return (
        <TextField disabled label="Mappable value" value={mappableValue} />
    )
}
...
function DisplayMappersSelection (
    { selectedMapper, mappers, onChange }: DisplayMappersSelectionProps
): ReactElement {
    return (
...
function DisplayMappedValue (
    { mapper, mappedValue, onChange }: DisplayMappedValueProps
): ReactElement {
