...
function Search ({ project, nextView, changeView }: SearchProps): ReactElement {
    ...
    function submitProject (): void {
        ...
        projectApi.uploadProjectFile(project.uid, selectedFile).catch(
            x => console.error('Failed to upload project file', x)
        )
        changeView(nextView)
    }
    ...
    return (
        <Fragment>
            <StepHeader
                title='Search'
                description='Parse search document'
            />
            <br />
            <Box sx={{ width: 300 }}>
                <Stack spacing={2}>
            ...
            <Dialog
                ...
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        Uploading a new search document will remove
                        existing items from project.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleDialogCancel}>Cancel</Button>
                    <Button onClick={handleDialogConfirm} autoFocus>Confirm</Button>
                </DialogActions>
            </Dialog>
        </Fragment>
    )
