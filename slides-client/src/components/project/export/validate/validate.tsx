...
export default function Validate ({ project }: ValidateProps): ReactElement {
    ...
    useEffect(() => {
        const getImagesWithThumbnail = (): void => {
            imageApi.getImagesWithThumbnail(project.uid)
                .then(images => {
                    setImages(images)
    ...
    return (
        <Container sx={{ vh: 100 }}>
            <StepHeader
                title='Validate'
                description='Validate exported images.'
            />
            <FormGroup>
                <FormLabel>Show</FormLabel>
                <FormGroup row>
                    <FormControlLabel
                        control={<Switch value={showIncluded}
                            checked={showIncluded} />}
                        label="Included"
                        onChange={(event, checked) => setShowIncluded(checked)}
                    />
                    <FormControlLabel
                        control={<Switch value={showExcluded}
            ...
            <Pagination
                ...
                color="primary"
            />
            {openedImage !== undefined && <ValidateImage
                open={imageOpen}
                image={openedImage}
