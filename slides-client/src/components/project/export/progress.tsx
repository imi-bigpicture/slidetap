...
export default function Progress ({ project }: ProgressProps): ReactElement {
    ...
    useEffect(() => {
        const getImages = (): void => {
            projectApi.getImages(project.uid, project.itemSchemas.filter(
                itemSchema => itemSchema.itemValueType === ItemType.IMAGE
            )[0].uid)
                .then(images => {
                    const completed = images.filter(image => image.status === ImageStatus.COMPLETED)
    ...
    const statusColumnFunction = (image: ImageTableItem): ReactElement => {
        if (image.status === ImageStatus.COMPLETED) {
            return <Chip label={ImageStatusStrings[image.status]} color='success' variant="outlined" />
        }
        if (image.status === ImageStatus.FAILED) {
            return <Chip label={ImageStatusStrings[image.status]} color='error' variant="outlined" />
        }
        if (image.status === ImageStatus.NOT_STARTED) {
            return <Chip label={ImageStatusStrings[image.status]} color='secondary' variant="outlined" />
        }
        return <Chip label={ImageStatusStrings[image.status]} color='primary' variant="outlined" />
    }
    ...
    return (
        <React.Fragment>
            <StepHeader
                title='Progress'
                description='Status of image export.'
            />
            <LinearProgressWithLabel value={progress} />
            {images !== undefined && <Table
                columns={[
                    {
                        header: 'Name',
                        accessorKey: 'name'
